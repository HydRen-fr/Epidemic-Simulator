# V6 - Asymptomatiques et distanciation sociale
...
TAILLE_CARRE = 700  # Taille du cadre où les individus évoluent
...
RAYON_DISTANCIATION = 10 # Rayon pour la distanciation sociale


# Pour le carré de quarantaine - proportionnel au grand carré
TAILLE_QUARANTAINE = 0.28 * TAILLE_CARRE  # Taille du cadre de la quarantaine
# Placement des individus dans la quarantaine
EMPLACEMENT_QUARANTAINE_X = ((LARGEUR_ECRAN - TAILLE_CARRE) // 2) + (TAILLE_CARRE * 1.085)
...
class Individu:
    ...
    def rebondir_sur_murs(self):
        ...
        if not self.est_en_quarantaine:
            if self.x - self.taille < (LARGEUR_ECRAN - TAILLE_CARRE) // 2:
                self.x = (LARGEUR_ECRAN - TAILLE_CARRE) // 2 + self.taille
                self.vitesse_x = -self.vitesse_x
            elif self.x + self.taille > (LARGEUR_ECRAN + TAILLE_CARRE) // 2:
                self.x = (LARGEUR_ECRAN + TAILLE_CARRE) // 2 - self.taille
                self.vitesse_x = -self.vitesse_x

            if self.y - self.taille < (HAUTEUR_ECRAN - TAILLE_CARRE) // 2:
                self.y = (HAUTEUR_ECRAN - TAILLE_CARRE) // 2 + self.taille
                self.vitesse_y = -self.vitesse_y
            elif self.y + self.taille > (HAUTEUR_ECRAN + TAILLE_CARRE) // 2:
                self.y = (HAUTEUR_ECRAN + TAILLE_CARRE) // 2 - self.taille
                self.vitesse_y = -self.vitesse_y
        else:
